<template>
    <template v-for="navMenu in navMenus" :key="navMenu.path">
        <template v-if="!navMenu.meta.isHide">
            <el-menu-item v-if="!hasChildren(navMenu)" :index="navMenu.path">
                <a v-if="navMenu.meta && navMenu.meta.isLink" :href="navMenu.path" target="_blank"
                    @click.stop='() => { }'></a>
...
const hasChildren = (item: Menu.MenuOptions) => {
    return item.children && !item.children.every(item => item.meta.isHide)
}
</script>
