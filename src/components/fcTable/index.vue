...
<template>
    <div class="fc-table-box" :style="{ 'height': _height }" ref="fcTableMain" v-loading="loading">
        ...
        <slot name="pagination">
            <div v-if="pagination">
                <el-pagination background layout="total, sizes, prev, pager, next, jumper" :current-page="pageable.page"
                    :page-size="pageable.pageSize" :total="pageable.total" :page-sizes="pageSizes"
...
const radio = ref("");          // 单选值
const tableColumns = reactive<ColumnProps[]>(props.columns);                        // 接收 columns 并设置为响应式
